...
class Example(QWidget):
    ...
    def initUI(self):
        ...
        self.address.move(50, 460)

        self.postal = QCheckBox('Почтовый индекс', self)
        self.postal.move(650, 220)
        self.postal.adjustSize()
        self.postal.clicked.connect(self.search)
    ...
    def show_img(self, search=False):
        if self.find_input.text() and search:
            ...
            res = response[0]['GeoObject']['metaDataProperty']['GeocoderMetaData']['Address']['formatted']
            if self.postal.isChecked():
                try:
                    res += ', Индекс: ' + response[0]['GeoObject']['metaDataProperty']['GeocoderMetaData']['Address'][
                        'postal_code']
                except KeyError:
                    res += ', Индекс не найден'
            self.address.setText(res)
