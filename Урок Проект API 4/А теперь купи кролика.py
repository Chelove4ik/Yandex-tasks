...
def handle_dialog(req, res):
    ...
    if req['session']['new']:
        ...
        res['response']['text'] = f'Привет! Купи {sessionStorage[user_id]["животное"]}а!'
        # Получим подсказки
    ...
    if any(i in req['request']['original_utterance'].lower() for i in [
        ...
        res['response']['text'] = f'{sessionStorage[user_id]["животное"]}а можно найти на Яндекс.Маркете!'
        if sessionStorage[user_id]["животное"] == 'кролик':
            ...
            res['response']['text'] = f'Купи {sessionStorage[user_id]["животное"]}а!'
            # Получим подсказки
            res['response']['buttons'] = get_suggests(user_id)
        return

    # Если нет, то убеждаем его купить слона!
    res['response']['text'] = f'Все говорят "%s", а ты купи {sessionStorage[user_id]["животное"]}а!' % (
...
def get_suggests(user_id):
    ...
    if len(suggests) < 2:
        suggests.append({
            "title": "Ладно",
            "url": f"https://market.yandex.ru/search?text={sessionStorage[user_id]['животное']}",
            "hide": True
        })
